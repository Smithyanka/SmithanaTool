...
from smithanatool_qt.tabs.parsers.kakao.shared.auth.session import get_session_path
...
def _collect_dom_urls(series_id: int, product_id: str | int, *,
    ...
    if ctx is not None:
        _collect_dom_urls_in_ctx(
            ctx,
            url,
            urls_json_path,
            log=log,
            stop_flag=stop_flag,
            pre_action=pre_action if callable(pre_action) else None,
            scroll_ms=scroll_ms,
        )
