...
DPM_72 = 2835  # 72 dpi in dots-per-meter
...
def register_memory_image(key: str, img: QImage) -> None:
    """Register a copy of an image under an arbitrary key (e.g. mem://...)."""
    if img is None:
        return
...
def _set_dpm_from_dpi(qimg: QImage, xdpi: float, ydpi: float | None = None) -> None:
...
def _ensure_dpm(qimg: QImage, fallback_dpi: float = 72) -> None:
